...
- **Move GitHub Actions off Node 20 runners** — **implemented 2026-07-30, awaiting verification.**
  ...
  pre-flight image-existence check, SSH host-key verification via a `SERVER_FINGERPRINT` secret,
  and env-var passing instead of `${{ }}` interpolation into the remote script.
  **Verified so far:** static check only — all six pinned SHAs resolve and every Node action
  ...
  main + post), the private submodule still checks out, and `:latest` does not move; then create
  the `SERVER_FINGERPRINT` secret and deploy. Effort: S.
